...
BBQ Age first-100, failures counted as wrong: the current fresh-session run scored **0.66** (ambiguous 0.46, disambiguated 0.86; p50 288 ms; 0 API/model errors). A prior run scored 0.76 (ambiguous 0.70, disambiguated 0.82), which illustrates that Apple FM quality can vary between runs. This is an Apple FM-specific evaluation result, not a claim of Jev-equivalent reasoning. Re-run `npm run eval -- bbq --limit 100` after prompt changes.
