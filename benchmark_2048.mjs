...
export function payloadFor(board, sessionId) {
...
export async function runGame(seed, endpoint, maxMoves = 100, session = false) {
  ...
  while (moves < maxMoves) {
    const payload = payloadFor(board, session ? `2048-seed-${seed}` : undefined); const valid = payload.state.valid_moves;
...
function options() {
  const args = process.argv.slice(2); const out = { endpoint: process.env.JEV_LOCAL_URL ?? "http://127.0.0.1:8787/v1/systemone", seeds: [1, 7, 42], maxMoves: 100, session: false };
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (args[index] === "--session") out.session = true;
  }
